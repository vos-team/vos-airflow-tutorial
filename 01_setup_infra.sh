...
NODEGROUP_NAME=ng-airflow-first
...
aws iam create-policy \
    ...
    --policy-document file://cloud-native-airflow-solution-on-amazon-eks/resources/efs-iam-policy-example.json
...
db_endpoint=$(aws rds create-db-cluster --region $AWS_REGION \
  ...
  --master-user-password rhdrksdmlrkcl \
  --storage-encrypted \
  --vpc-security-group-ids $db_security_group_id \
  --query 'DBCluster.Endpoint' --output text)

# 위의 password는 내가 임의로 적은 공간의가치 영어타자

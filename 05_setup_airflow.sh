...
kubectl create secret generic airflow-db \
   -n airflow \
   --from-literal=connection=postgresql://postgres:rhdrksdmlrkcl@${DB_ENDPOINT}:5432/postgres

kubectl create secret generic airflow-result-db \
   -n airflow \
   --from-literal=connection=db+postgresql://postgres:rhdrksdmlrkcl@${DB_ENDPOINT}:5432/postgres
